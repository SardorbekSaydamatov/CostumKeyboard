//
//  CostumTextFieldwithKeyboard.swift
//  CostumKeyboard 2
//
...
struct CustomTextFieldWithKeyboard<TextField: View, Keyboard: View>: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIHostingController<TextField> {
        ...
        DispatchQueue.main.async {
            if let textField = controller.view.allSuview.first(where: {$0 is UITextField}) as? UITextField, textField.inputView == nil {
...
fileprivate extension UIView {
    var allSuview: [UIView] {
        return self.subviews.flatMap({ [$0] + $0.allSuview} )
    }
}
